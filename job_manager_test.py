...
class JobManager:
    ...
    def create_job_script(self, job_index):
        ...
        with open(job_script_name, 'w') as script_file:
            ...
            script_file.write(f"#SBATCH --job-name=test_job_{job_index}\n")
            script_file.write(f"#SBATCH --cpus-per-task={self.cpus_per_job}\n")
            script_file.write(f"#SBATCH --output={os.path.join(self.temp_data_path, f'%j_TEST_OUTPUT_{job_index}.txt')}\n")
            script_file.write(f"#SBATCH --mem={self.mem}G\n")
            script_file.write(f"export PYTHONPATH={self.parent_path}:$PYTHONPATH\n")
            script_file.write(f"{self.env_path} {self.python_script_path} {job_index}\n")
        # print(monitor_file)
...
if __name__ == "__main__":
    python_script_path = "/home/roeeidan/icarus_framework/client.py"
