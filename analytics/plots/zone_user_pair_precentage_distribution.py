...
def find_and_process_data(base_path):
    ...
    total_attacks = 0
    total_attack_dict = {}
    file_path = "/home/roeeidan/icarus_framework/analytics/outputs/zone_distribution/distribution_user_pair_precentage_appearance_zone.png"
                        ...
                        if "ZAtk" in loader.data_cache:
                            ...
                            for data in datas.values():
                                ...
                                for item in atkflowset:
                                    if item[0] not in dict_show_up.keys():
                                        dict_show_up[item[0]] = 1
                                merge_integer_dictionaries(total_attack_dict, dict_show_up)
                dict_normalizer(total_attack_dict,total_attacks)
                print(len(total_attack_dict))
                import pickle
                with open('/home/roeeidan/icarus_framework/analytics/outputs/total_attack_dict.pkl', 'wb') as file:
                    # Step 4: Use pickle.dump() to serialize and write the dictionary to the file
                    pickle.dump(total_attack_dict, file)
