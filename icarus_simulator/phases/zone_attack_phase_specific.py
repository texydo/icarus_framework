...
class ZoneAttackSpecificPhase(BasePhase):
    def __init__(
        self,
        read_persist: bool,
        persist: bool,
        ...
        self.outs: List[Pname] = [zatkp_out]
    ...
    def _compute(
        ...
        zone_pairs = [
            (717, 1361),  # Center keys for the first zone pair (key)
            (3061, 3282),  # Center keys for the second zone pair (key2)
            (1442, 3281)   # Center keys for the third zone pair (key3)
        ]

        if self.run_jobs:
            job_name = "ZoneAttackSpecificData"
